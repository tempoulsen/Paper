...
Four transparent price-based benchmarks show how much of the learned performance is attainable with interpretable signals. Medium-horizon continuation follows the past-return logic of \textcite{jegadeeshReturnsBuyingWinners1993a} and uses the cumulative return over 252 trading days after skipping the most recent 21 days,
\begin{equation}
 z^{\mathrm{CONT}}_{i,t}=\prod_{j=22}^{273}(1+r_{i,t-j})-1.
 \label{eq:cont}
\end{equation}
We denote this implemented rule CONT. Its 252 compounded returns start after a 21-day skip, so the oldest observation lies 273 trading days in the past; the label avoids equating this exact window with conventional 12-minus-1 momentum.
