\section{Re-estimating and extending the representation comparison}\label{sec:mechanism_results}

\Textcite{reimagining_price_trends_jiang} compare logistic classifiers and CNN1D across alternative scaling schemes and show that image-scaled CNN1D can match the portfolio performance of the chart CNN. We first assess whether this representation result persists through 2024. We then extend the portfolio-level comparison with direct tests of learned-model rank overlap, conditional predictive content, and multimodal complementarity.

\subsection{Extended-sample scaling and temporal sequence models}\label{subsec:scaling_results}\label{subsec:temporal_results}
...
The longer-horizon grid shows an important boundary. Local high--low scaling remains the strongest logistic representation, but temporal convolution adds little once the forecast horizon extends beyond one week. In $I20/R20$, the image-scale logistic classifier reaches a Sharpe ratio of 2.37, compared with 2.15 for CNN1D, and the difference becomes more pronounced as quarterly portfolio performance weakens. Local high--low scaling therefore remains informative across horizons, while the additional contribution of temporal learning is concentrated at the weekly horizon.
...
\subsection{Extension: complementarity across representations}\label{subsec:fusion_results}

The second extension tests complementarity directly by combining chart and sequence information. In the central $I20/R5$ specification, the multimodal model reaches a Sharpe ratio of 6.48, compared with 6.52 for the chart CNN and 6.55 for CNN1D, and the best single branch remains strongest in the other weekly specifications. The same pattern appears after separate training: late fusion of the chart CNN and CNN1D probabilities or logits produces weekly performance close to the individual branches. Detailed comparisons appear in the online appendix.

Across feature-level and prediction-level designs, fusion preserves strong weekly portfolio performance but does not improve on the best individual branch. Combined with the score correlations and joint regressions, the evidence points to substantial but incomplete overlap and limited complementarity between the chart and sequence representations.
